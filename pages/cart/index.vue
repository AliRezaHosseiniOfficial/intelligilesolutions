      ...
      <section class="">
        <div class="container px-sm-5 pb-5">
          <div class="d-flex justify-content-between align-items-center">
            <h1>{{ title }}</h1>
            <span class="link h4">{{ carts.length }} Items</span>
          </div>
          <div class="mt-2 mb-4">
            <ProductItemBox v-for="item in carts" :key="item.id"
                            :data="{productID: item.acf.productid}" />
          </div>
...
export default {
    ...
    items: [
      {
        ...
        title: "Sella body butter type black fpr face skin",
        desc: "20 ml",
        count: 1,
        discount: 0.12,
        price: 250,
  ...
  async asyncData() {
    const config = {
      method: 'get',
      url: "https://api.intelligilesolutions.com/wp-json/wp/v2/cart",
      headers: {
